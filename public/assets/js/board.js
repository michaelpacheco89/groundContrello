...
$(document).on("submit", ".editListForm, .editTaskForm", function(event) {
    ...
    if ($form.hasClass("editListForm")) {
        ...
        //new code
        newContent.append("<i class='fa fa-plus assignTask' aria-hidden='true' style='position: relative;float: right;top:2px;'></i>");
        //new
...
$(document).on("click", ".addCardLink", function() {
    ...
    //new
    var add = $("<i class='fa fa-plus assignTask' aria-hidden='true' style='position: relative;float: right;top:2px;'></i>");
    //end new
    var remove = $("<i class='fa fa-times closeAddCard' aria-hidden='true' style='position: relative;top:2px;'></i>");
    if ($(this).attr('value')) {
        input.val($(this).attr('value'));
    }
    form.append(input, button, add, remove);
